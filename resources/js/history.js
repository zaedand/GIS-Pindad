                                        ...
                                        return `
                                            <tr class="border-b border-gray-200 hover:bg-white">
                                                <td class="py-3 px-4 text-gray-800">
                                                    ${formatWIBTime(item.timestamp || item.created_at)}
                                                </td>
                                                <td class="py-3 px-4">
...
// Enhanced Device History Tracker Class
...
// Enhanced Phone Monitoring Class with Pagination
class PhoneMonitoring {
    ...
    filterPhones(status) {
        ...
        phoneStatusPagination.filteredPhones = status === 'all' ? currentPhoneData :
            currentPhoneData.filter(phone => phone.status === status);

        this.updateFilterInfo(status, phoneStatusPagination.filteredPhones.length, currentPhoneData.length);

        // Calculate pagination for phones
        phoneStatusPagination.totalPages = Math.ceil(phoneStatusPagination.filteredPhones.length / phoneStatusPagination.itemsPerPage);
        phoneStatusPagination.currentPage = 1; // Reset to first page
    ...
    renderPhoneGrid() {
        ...
        if (phoneStatusPagination.filteredPhones.length === 0) {
            phoneGrid.classList.add('hidden');
            if (noResults) noResults.classList.remove('hidden');
            updatePhonePaginationControls();
            return;
        }

        // Calculate pagination
        const startIndex = (phoneStatusPagination.currentPage - 1) * phoneStatusPagination.itemsPerPage;
        const endIndex = startIndex + phoneStatusPagination.itemsPerPage;
        const phonesToShow = phoneStatusPagination.filteredPhones.slice(startIndex, endIndex);

        phoneGrid.classList.remove('hidden');
        if (noResults) noResults.classList.add('hidden');
        phoneGrid.innerHTML = phonesToShow.map(phone => this.generatePhoneCard(phone)).join('');
            ...
            <div class="bg-white rounded-xl p-4 shadow-lg border-l-4 border-${color}-500 hover:shadow-xl transition-all duration-300 transform hover:-translate-y-1">
                ...
                <div class="space-y-2">
                    <div class="flex justify-between items-center text-xs">
                        ...
                        <span class="text-gray-700 font-medium">${formatWIBTime(phone.lastSeen.toISOString())}</span>
                    </div>
    ...
    initLogboard() {
        const currentPhoneData = this.phoneData;
        phoneStatusPagination.filteredPhones = currentPhoneData;
        phoneStatusPagination.totalPages = Math.ceil(phoneStatusPagination.filteredPhones.length / phoneStatusPagination.itemsPerPage);
...
// Utility function to get offline phones
...
window.goToPhonePage = goToPhonePage;
            ...
            <div class="p-6 overflow-y-auto max-h-[75vh]">
                <div class="grid grid-cols-1 md:grid-cols-5 gap-4 mb-6">
                    ...
                    <div class="bg-red-50 p-4 rounded-xl border border-red-200">
                        ...
                        <div class="text-2xl font-bold text-re// resources/js/logs.js - Optimized with Pagination and WIB Time Format
...
// Enhanced Pagination settings for both Activity Logs and Phone Status
let activityLogPagination = {
    currentPage: 1,
    itemsPerPage: 10,
    totalPages: 1,
    filteredLogs: []
    ...
    try {
        ...
        if (response.ok) {
            ...
            disconnectLogs = logs.map(log => ({
                ...
                time: log.timestamp || log.created_at, // Keep original WIB format
...
// Optimized time formatting for WIB (already in correct format from server)
function formatWIBTime(timeString) {
    // Server already sends time in WIB format: '2025-07-29 10:43:53'
    // Just format it for better display
    ...
    try {
        ...
        if (typeof timeString === 'string' && timeString.includes(' ') && timeString.length === 19) {
            return timeString.replace(' ', ' • '); // Add separator for better readability
        }
...
function setupEventListeners() {
    ...
    const activityItemsPerPage = document.getElementById('activity-items-per-page');
    if (activityItemsPerPage) {
        activityItemsPerPage.addEventListener('change', (e) => {
            activityLogPagination.itemsPerPage = parseInt(e.target.value);
            activityLogPagination.currentPage = 1;
            renderActivityLog();
        });
    }

    // Phone Status Pagination
    const phoneItemsPerPage = document.getElementById('phone-items-per-page');
    if (phoneItemsPerPage) {
        phoneItemsPerPage.addEventListener('change', (e) => {
            phoneStatusPagination.itemsPerPage = parseInt(e.target.value);
            phoneStatusPagination.currentPage = 1;
            if (phoneMonitoring) {
                phoneMonitoring.updateLogboard();
            }
        });
...
// Go to specific phone status page
function goToPhonePage(page) {
    if (page < 1 || page > phoneStatusPagination.totalPages) return;
    phoneStatusPagination.currentPage = page;
    if (phoneMonitoring) {
        phoneMonitoring.updateLogboard();
    }
}
...
function initializeSearch() {
    ...
    if (searchContainer) {
        searchContainer.innerHTML = `
            ...
            <!-- Activity Log Pagination Controls Container -->
            <div id="activity-pagination-controls" class="mb-4"></div>
        `;
                            ...
                            ${sortedStats.map((stats, index) => `
                                <tr class="border-b border-gray-200 hover:bg-gray-50">
                                    ...
                                    <td class="py-3 px-4 text-xs text-gray-500">
                                        ${stats.lastActivity ? formatWIBTime(stats.lastActivity) : 'N/A'}
                                    </td>
                                    <td class="py-3 px-4">
                                        <div class="flex gap-1">
                                            <button
...
function displayEndpointHistoryModal(endpoint, data) {
    ...
    modal.innerHTML = `
        <div class="bg-white rounded-2xl max-
            phoneStatusPagination.currentPage = 1;
            if (phoneMonitoring) {
                phoneMonitoring.updateLogboard();
            }
...
function applyLiveStatus() {
    ...
    latestStatus.forEach(update => {
        ...
        // Keep the original WIB timestamp from server
...
async function trackStatusChangeAndSave(devices) {
    ...
    for (const device of devices) {
        ...
        // Use the timestamp directly from server (already in WIB format)
        ...
        if (prevStatus && prevStatus !== currentStatus) {
            ...
            const logEntry = {
                ...
                from: prevStatus,
                to: currentStatus,
                time: timestamp, // Keep original WIB format
            ...
            if (existingLogIndex === -1) {
                disconnectLogs.unshift(logEntry);
...
// ENHANCED: Activity log rendering with improved pagination
function renderActivityLog() {
    ...
    activityLogPagination.filteredLogs = disconnectLogs.filter(log => {
        ...
        // Enhanced date filtering for WIB format
        const logDate = log.time.split(' ')[0]; // Extract date part from "2025-07-29 10:43:53"
    ...
    activityLogPagination.filteredLogs.sort((a, b) => new Date(b.time) - new Date(a.time));

    console.log('Filtered logs:', activityLogPagination.filteredLogs.length, 'logs to show');

    // Calculate pagination
    activityLogPagination.totalPages = Math.ceil(activityLogPagination.filteredLogs.length / activityLogPagination.itemsPerPage);
    const startIndex = (activityLogPagination.currentPage - 1) * activityLogPagination.itemsPerPage;
    const endIndex = startIndex + activityLogPagination.itemsPerPage;
    const logsToShow = activityLogPagination.filteredLogs.slice(startIndex, endIndex);

    // Clear container
    container.innerHTML = '';

    if (activityLogPagination.filteredLogs.length === 0) {
        container.innerHTML = `
    ...
    // Render current page logs with optimized time display
    ...
    updateActivityPaginationControls();
    updateSearchResultsCount(activityLogPagination.filteredLogs.length, disconnectLogs.length);
}

// Enhanced Activity Log Pagination Controls
function updateActivityPaginationControls() {
    const paginationContainer = document.getElementById('activity-pagination-controls');
    if (!paginationContainer) return;

    if (activityLogPagination.totalPages <= 1) {
        paginationContainer.innerHTML = '';
        return;
    }

    const startItem = ((activityLogPagination.currentPage - 1) * activityLogPagination.itemsPerPage) + 1;
    const endItem = Math.min(activityLogPagination.currentPage * activityLogPagination.itemsPerPage, activityLogPagination.filteredLogs.length);

    let paginationHTML = `
        <div class="flex items-center justify-between bg-white px-4 py-3 rounded-lg border shadow-sm">
            <div class="flex items-center gap-4 text-sm text-gray-700">
                <span>Menampilkan ${startItem} - ${endItem} dari ${activityLogPagination.filteredLogs.length} aktivitas</span>
                <select id="activity-items-per-page" class="px-3 py-1 border border-gray-300 rounded text-sm">
                    <option value="5" ${activityLogPagination.itemsPerPage === 5 ? 'selected' : ''}>5 per halaman</option>
                    <option value="10" ${activityLogPagination.itemsPerPage === 10 ? 'selected' : ''}>10 per halaman</option>
                    <option value="20" ${activityLogPagination.itemsPerPage === 20 ? 'selected' : ''}>20 per halaman</option>
                    <option value="50" ${activityLogPagination.itemsPerPage === 50 ? 'selected' : ''}>50 per halaman</option>
                </select>
            </div>

            <div class="flex gap-1">
                <button onclick="goToActivityPage(${activityLogPagination.currentPage - 1})"
                        ${activityLogPagination.currentPage <= 1 ? 'disabled' : ''}
    ...
    let startPage = Math.max(1, activityLogPagination.currentPage - Math.floor(maxVisiblePages / 2));
    let endPage = Math.min(activityLogPagination.totalPages, startPage + maxVisiblePages - 1);
    ...
    for (let i = startPage; i <= endPage; i++) {
        paginationHTML += `
            <button onclick="goToActivityPage(${i})"
                    class="px-3 py-1 border border-gray-300 rounded text-sm hover:bg-gray-50 ${i === activityLogPagination.currentPage ? 'bg-indigo-500 text-white border-indigo-500' : ''}">
                ${i}
            </button>
        `;
    }
    ...
    paginationHTML += `
                <button onclick="goToActivityPage(${activityLogPagination.currentPage + 1})"
                        ${activityLogPagination.currentPage >= activityLogPagination.totalPages ? 'disabled' : ''}
                        class="px-3 py-1 border border-gray-300 rounded text-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
    ...
    const itemsPerPageSelect = document.getElementById('activity-items-per-page');
...
// Go to specific activity log page
function goToActivityPage(page) {
    if (page < 1 || page > activityLogPagination.totalPages) return;
    activityLogPagination.currentPage = page;
    renderActivityLog();
}

// Enhanced Phone Status Pagination Controls
function updatePhonePaginationControls() {
    const paginationContainer = document.getElementById('phone-pagination-controls');
    if (!paginationContainer) return;

    if (phoneStatusPagination.totalPages <= 1) {
        paginationContainer.innerHTML = '';
        return;
    }

    const startItem = ((phoneStatusPagination.currentPage - 1) * phoneStatusPagination.itemsPerPage) + 1;
    const endItem = Math.min(phoneStatusPagination.currentPage * phoneStatusPagination.itemsPerPage, phoneStatusPagination.filteredPhones.length);

    let paginationHTML = `
        <div class="flex items-center justify-between bg-white px-4 py-3 rounded-lg border shadow-sm">
            <div class="flex items-center gap-4 text-sm text-gray-700">
                <span>Menampilkan ${startItem} - ${endItem} dari ${phoneStatusPagination.filteredPhones.length} telepon</span>
                <select id="phone-items-per-page" class="px-3 py-1 border border-gray-300 rounded text-sm">
                    <option value="5" ${phoneStatusPagination.itemsPerPage === 5 ? 'selected' : ''}>5 per halaman</option>
                    <option value="10" ${phoneStatusPagination.itemsPerPage === 10 ? 'selected' : ''}>10 per halaman</option>
                    <option value="20" ${phoneStatusPagination.itemsPerPage === 20 ? 'selected' : ''}>20 per halaman</option>
                    <option value="50" ${phoneStatusPagination.itemsPerPage === 50 ? 'selected' : ''}>50 per halaman</option>
                </select>
            </div>

            <div class="flex gap-1">
                <button onclick="goToPhonePage(${phoneStatusPagination.currentPage - 1})"
                        ${phoneStatusPagination.currentPage <= 1 ? 'disabled' : ''}
                        class="px-3 py-1 border border-gray-300 rounded text-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                    <i class="fas fa-chevron-left"></i> Prev
                </button>
    `;
    ...
    // Page numbers
    const maxVisiblePages = 5;
    let startPage = Math.max(1, phoneStatusPagination.currentPage - Math.floor(maxVisiblePages / 2));
    let endPage = Math.min(phoneStatusPagination.totalPages, startPage + maxVisiblePages - 1);
    ...
    for (let i = startPage; i <= endPage; i++) {
        paginationHTML += `
            <button onclick="goToPhonePage(${i})"
                    class="px-3 py-1 border border-gray-300 rounded text-sm hover:bg-gray-50 ${i === phoneStatusPagination.currentPage ? 'bg-indigo-500 text-white border-indigo-500' : ''}">
                ${i}
            </button>
        `;
    }
    ...
    paginationHTML += `
                <button onclick="goToPhonePage(${phoneStatusPagination.currentPage + 1})"
                        ${phoneStatusPagination.currentPage >= phoneStatusPagination.totalPages ? 'disabled' : ''}
                        class="px-3 py-1 border border-gray-300 rounded text-sm hover:bg-gray-50 disabled:opacity-50 disabled:cursor-not-allowed">
                    Next <i class="fas fa-chevron-right"></i>
                </button>
            </div>
    ...
    // Re-attach event listener for items per page
    const itemsPerPageSelect = document.getElementById('phone-items-per-page');
    if (itemsPerPageSelect) {
        itemsPerPageSelect.addEventListener('change', (e) => {
            phoneStatusPagination.itemsPerPage = parseInt(e.target.value);
// Continuation of logs.js from the paste.txt
